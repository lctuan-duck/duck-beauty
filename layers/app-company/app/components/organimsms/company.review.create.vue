...
const schema = v.object({
  summary: v.pipe(
    v.string(),
    v.minLength(5, "Phải có ít nhất 5 ký tự"),
    v.maxLength(100, "Tối đa là 100 ký tự")
  ),
  tags: v.pipe(
    v.array(v.string()),
    v.minLength(1, "Phải có ít nhất 1 tag"),
    v.maxLength(5, "Tối đa là 5 tags")
  ),
});
...
function handleUpdateContentText(contentText: string) {
  state.contentText = contentText;
  editorState.isEditorEmpty = contentText.length === 0;
}
...
async function handleSubmit() {
  if (editorRef.value?.editor?.getText().trim().length === 0) {
    editorState.isEditorEmpty = true;
    return;
  }
    ...
    >
      ...
      >
        <DuckEditor
          ...
          @update:content-text="handleUpdateContentText"
          @blur="
            ({ isEmpty }) => {
